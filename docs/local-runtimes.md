...
CopyTranslator now treats local translators as registry entries instead of hard-coding Hy-MT2.
...
```zsh
~/.config/copy-translator/local-models.json
```
...
```zsh
swift run CopyTranslator --list-local-models --custom-local-models /path/to/local-models.json
```
